...
def Maneuver_1(aTargetAltitude):
   ...
   # UAVS.commands.takeoff(aTargetAltitude) # Takeoff to target alttude
    ...
    UAVS.airspeed = 0.5 #m/s

    point1 = LocationGlobalRelative(0, 5, aTargetAltitude) #dNorth, dEast, Altitude
    print "Vehicle is moving 0 m North, 3 m East"
    UAVS.simple_goto(point1, groundspeed=1)
    UAVS.flush()
    time.sleep(3)
    
    point2 = LocationGlobalRelative(0, -5, aTargetAltitude) #dNorth, dEast, Altitude
    print "Vehicle is moving 0 m South, 3 m West"
    UAVS.simple_goto(point2, groundspeed=1)
    UAVS.flush()
    time.sleep(2)
